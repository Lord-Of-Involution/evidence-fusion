...
def main(args):
    ...
    print(f"Firing up GPUs on {device}... Engaging Topology: {args.geometry.upper()}")
    ...
    model = EvidenceGNN(r_link=args.r_link, hidden_dim=args.hidden_dim, geometry=args.geometry).to(device)
    # [CRITICAL ARMOR] Low weight decay to prevent micro-gradient annihilation
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        for data in pbar:
            ...
            if epoch < args.bce_epochs:
                # [CRITICAL FIX] Strict removal of c_prior during BCE pre-heating.
                # Forces network to break symmetry and learn the class imbalance natively.
                logits = l_pop_transform(f_x, alpha=2.0)
                loss = torch.nn.BCEWithLogitsLoss()(logits, data.y.float().view_as(logits))
                desc_loss = "BCE"
            else:
                # 1-POP phase: Re-engage strict Bayesian inference with prior
        ...
        print(f"Ep {epoch} | Tr Loss: {train_loss/len(train_loader):.6f} | Val Loss: {avg_val_loss:.6f} | Acc: {acc:.6f}", flush=True)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--bce_epochs", type=int, default=10, help="Number of epochs to train with BCE loss")
    ...
    parser.add_argument("--geometry", type=str, default="plane_parallel", choices=["plane_parallel", "lightcone"], help="Physical topology mapping")
